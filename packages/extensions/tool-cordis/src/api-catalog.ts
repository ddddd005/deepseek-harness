...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'prepareConversationRequest(sessionId: SessionId): Promise<GenerateOptions>',
        description: 'Assemble a live session\'s next conversation surface without mutating the Session or entering the LLM request lifecycle.',
        parameters: [{ name: 'sessionId', description: 'identity shared by the live session and its loop agent.' }],
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'replaceStreamRequest(original: GenerateOptions, replacement: GenerateOptions): void',
        description: 'Register the payload that the adapter will receive for one in-flight stream request. The replacement keeps the routing, call configuration, session, purpose, cancellation signal, and every unrecognized property identical; only `system`, `messages`, and `tools` may change. A request accepts one replacement, so independently composed controllers cannot silently erase each other\'s finalized payload.',
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'catalog(scope?: ScopeKey, options: ToolCatalogOptions = {}): ToolCatalog',
        description: 'Read the registered tool schemas and their owning plugin/scope. This is a pre-dispatch registry view: it neither changes a schema nor claims to be the final Adapter payload.',
        parameters: [{ name: 'scope', description: 'the viewing scope (the agent); omitted = the global view.' }, { name: 'options', description: 'include registrations shadowed by nearer scopes.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    parameters: [{ name: 'options', description: 'the full request. A LOOP-built request carries the process-local {@link markAgentLoopRequest} identity and arrives deep-frozen (mutation throws): its content is a pure function of the session log. A listener that must control its payload registers a constrained replacement through {@link LlmRuntime.replaceStreamRequest}, then calls `next()`. Hand-built calls do not carry that marker; their messages already obey the immutable creation contract.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export interface ToolCatalogEntry {\n    readonly name: string;\n    readonly source: ToolRegistrationSource;\n    readonly schema: ToolSchema;\n    readonly effective: boolean;\n    readonly shadowedBy?: ScopeKey;\n}',
  },
  {
    name: 'ToolCatalogOptions',
    declaration: 'export interface ToolCatalogOptions {\n    readonly includeShadowed?: boolean;\n}',
  },
  {
    name: 'ToolDefinition',
  ...
  {
    name: 'ToolRegistrationSource',
    declaration: 'export interface ToolRegistrationSource {\n    readonly ownerPackage: string;\n    readonly scope?: ScopeKey;\n}',
  },
  {
