...
| `ctx.promptControl` | `core` | [`prompt-control`](../packages/prompt/prompt-control) | - | - | - | Freezes the evaluated read-only catalog over the system-prompt registry; profile storage, rules, and finalization land behind the same service. |
