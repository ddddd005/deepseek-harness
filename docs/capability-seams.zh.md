...
flowchart LR
  ...
  pkg_prompt_control["prompt-control"]
  svc_promptControl["ctx.promptControl<br/>Prompt Control catalog service"]
  ...
  pkg_prompt_control --> svc_promptControl
...
| `ctx.promptControl` | `core` | [`prompt-control`](../packages/prompt/prompt-control) | - | - | - | 在 system-prompt 注册表之上冻结已求值的只读 catalog；Profile 存储、规则与最终化落于同一服务之后。 |
